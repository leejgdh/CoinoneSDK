...
namespace CoinoneSDKV2UnitTest
{
    ...
    public class MyLimitOrdersUnitTest
    {
        ...
        public async Task MyLimitOrdersTest(ECoinCurrency currency)
        {
            try
            {
                RequestMyLimitOrders request_orders = new RequestMyLimitOrders(currency);

                var orders_res = await _coinoneClient.SendRequestAsync<ResponseMyLimitOrders>(request_orders);

                Assert.AreEqual(orders_res.IsSuccess, true, orders_res.Message);
            }
