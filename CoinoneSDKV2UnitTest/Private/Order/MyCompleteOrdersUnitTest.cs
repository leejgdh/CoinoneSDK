...
namespace CoinoneSDKV2UnitTest
{
    ...
    public class MyCompleteOrderUnitTest
    {
        ...
        public async Task MyCompleteOrderTest(ECoinCurrency currency)
        {
            try
            {
                RequestMyCompleteOrders request_complete_orders = new RequestMyCompleteOrders(currency);

                var complete_orders_res = await _coinoneClient.SendRequestAsync<ResponseMyCompleteOrders>(request_complete_orders);

                Assert.AreEqual(complete_orders_res.IsSuccess, true, complete_orders_res.Message);
            }
